  ...
  return (
    <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <nav style={{ borderBottom: '1px solid var(--border)', padding: '10px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <Link href="/roast" style={{ fontSize: 10, color: 'var(--dimmer)', textDecoration: 'none', letterSpacing: '0.1em' }}>
          $ ./roast.sh →
        </Link>
      </nav>
      ...
      <div style={{ flex: 1, padding: '32px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <div style={{ width: '100%', maxWidth: 560, marginBottom: 28 }} className="animate-slide-up">
          <div style={{ fontSize: 9, color: 'var(--dimmer)', letterSpacing: '0.15em', marginBottom: 6 }}>
            CHAINSNARK::MODULE — ORACLE_ENGINE v1.0
          </div>
          ...
          <p style={{ fontSize: 11, color: 'var(--dimmer)', lineHeight: 1.7 }}>
            // Ask anything. GenLayer AI delivers a brutal honest verdict
            with real reasoning. No filter. No bias. Stored permanently on-chain.
          </p>
        </div>
          ...
          <div style={{ width: '100%', maxWidth: 560 }} className="animate-fade-in">
            ...
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', padding: '16px' }}>
              ...
              <div style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 9, color: 'var(--dimmer)', letterSpacing: '0.12em', marginBottom: 8 }}>
                  # EXAMPLE QUERIES:
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {EXAMPLES.map(ex => (
                    <button key={ex} onClick={() => setQuestion(ex)}
                      style={{ fontSize: 9, padding: '3px 8px', border: '1px solid var(--border)', background: 'transparent', color: 'var(--dimmer)', cursor: 'pointer', letterSpacing: '0.04em', transition: 'all 0.15s' }}
              ...
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 10, color: 'var(--dimmer)', letterSpacing: '0.12em', marginBottom: 6 }}>
                  # QUERY_STRING (min 10 chars)
                </div>
                <div style={{ display: 'flex', gap: 0, alignItems: 'flex-start' }}>
                  <span style={{ padding: '10px 12px', background: 'rgba(0,229,255,0.04)', border: '1px solid var(--border2)', borderRight: 'none', fontSize: 12, color: 'var(--cyan)', whiteSpace: 'nowrap' }}>
                    ?_
                  </span>
                  <textarea
                    ...
                    onFocus={e => { e.target.style.borderColor = 'var(--cyan)'; e.target.style.boxShadow = '0 0 0 2px rgba(0,229,255,0.1)' }}
                    onBlur={e => { e.target.style.borderColor = 'var(--border2)'; e.target.style.boxShadow = 'none' }}
                  />
                </div>
                <div style={{ textAlign: 'right', fontSize: 9, color: question.length < 10 ? 'var(--red)' : 'var(--dimmer)', marginTop: 4, letterSpacing: '0.08em' }}>
              ...
              <button
                ...
                disabled={!isValid}
                className="btn-exec"
                style={{ width: '100%', fontSize: 12, padding: '12px', background: 'var(--cyan)', color: '#000' }}
              >
              ...
              <div style={{ marginTop: 10, fontSize: 9, color: 'var(--dimmer)', letterSpacing: '0.08em' }}>
                // FREE · AI CONSENSUS ON GENLAYER · TAKES 2-5 MIN
              </div>
            </div>
          </div>
        )}
        ...
        {stage === 'result' && result && (
          <div style={{ width: '100%', maxWidth: 560 }} className="animate-slide-up">
            <div style={{ marginBottom: 12, fontSize: 10, color: 'var(--cyan)', letterSpacing: '0.1em', display: 'flex', alignItems: 'center', gap: 8 }}>
